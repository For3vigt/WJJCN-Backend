...
def lambda_handler(event, context):
    ...
    # Check if token is send 
    if event.get('base_url', None) == None:
        return {
            'statuscode': 400,
            'body': json.dumps('base_url is not defined!')
        }
    
    # Check if token is send 
    if event.get('scrape', None) == None:
        return {
            'statuscode': 400,
            'body': json.dumps('scrape is not defined!')
        }

        # Check if token is send 
    if event.get('url_to_scrape', None) == None:
        return {
            'statuscode': 400,
            'body': json.dumps('url_to_scrape is not defined!')
        }
    ...
    if (token == token_from_website):
        # Collect the data 
        retailerName = '{}'.format(event['name'])
        retailerBase_url = '{}'.format(event['base_url'])
        retailerScrape = '{}'.format(event['scrape'])
        retailerUrl_to_scrape = '{}'.format(event['url_to_scrape'])
        retailerdict = { "name": retailerName, "base_url": retailerBase_url, "scrape": retailerScrape, "url_to_scrape": retailerUrl_to_scrape }

        newRetailer = retailerscol.insert_one(retailerdict)
        newRetailer_json = json.loads(json_util.dumps(newRetailer.inserted_id))
        return (newRetailer_json)
